...
logs, using the optional OmniRoute gateway, troubleshooting, and uninstalling
cleanly. It does not cover running from source — for that see the quick start in
...
- **The app is keyless, but a fresh install still needs a provider.** Asha ships
  with **no** provider API key of ours. A managed plan's keys live on the
  **Asha proxy** (hosted on Cloudflare) and are reached through a gateway; a BYOK
  key is yours and stays on your Mac. If you are not using a plan and have not
  configured a key, Asha has no brain to talk to — see
  [You must provide a model provider](#you-must-provide-a-model-provider).
- **OmniRoute is not included.** The local gateway is an **optional add-on you
...
points the brain at a *local* OmniRoute gateway
(`JARVIS_BRAIN_TRANSPORT=omniroute` in `Asha.app/Contents/Resources/app/.env`),
but OmniRoute is **not bundled**, so out of the box there is nothing for the
brain to talk to. Before Asha can think, point it at a provider with your own
key. This is a small config change in
`Asha.app/Contents/Resources/app/.env` — the exact file the app reads; the
first-run setup screen writes keys to that same file.
...
`_resolve_brain_transport`): `JARVIS_BRAIN_BASE_URL` (with
`JARVIS_BRAIN_API_KEY`) works for any OpenAI-compatible endpoint, which is how
OpenRouter and most other providers are used. If your provider names the model
differently, override it with `JARVIS_BRAIN_MODEL` (the shipped default is
`deepseek-v4.1-flash`).

**Installing OmniRoute yourself is optional.** If you would rather run a local
gateway, install it separately and keep `JARVIS_BRAIN_TRANSPORT=omniroute` —
see [The optional OmniRoute add-on](#the-optional-omniroute-add-on). It is one
option among these, not a requirement.
...
```text
[LLM] brain transport: transport=omniroute base=http://127.0.0.1:20128/v1 host=127.0.0.1:20128 model=deepseek-v4.1-flash key_env=(none)
```
...
1. Open the [releases page](https://github.com/atuljaiswal1246/jarvis/releases)
...
| Shipped non-secret config (model, transport, local gateway URL) | `Asha.app/Contents/Resources/app/.env` |
...
local gateway base URL. Your plan token is not in the bundle: sign-in stores it
...
```text
[config] keyless build: no provider API keys in app/.env; loaded: JARVIS_BRAIN_MODEL=deepseek-v4.1-flash, JARVIS_BRAIN_TRANSPORT=omniroute, OMNIROUTE_BASE_URL=http://127.0.0.1:20128/v1
```

## The optional OmniRoute add-on

Asha does **not** bundle its local gateway. OmniRoute is third-party software
that a user installs separately if they want it. That is a deliberate choice:

- It is large — roughly **2.1 GB unpacked** with its `node_modules` (Next.js) —
  and its `npm install` needs more memory than some machines have, so bundling
  it was not reliable to build.
- It is third-party software we would have to vendor, notice and keep updated.
- It is **not required** to use Asha. The app's own transports talk to a
  provider directly with your own key.

If you want it:

1. Install OmniRoute yourself (npm; see
   <https://www.npmjs.com/package/omniroute>) and run it. Version 3.8.50 was
   tested for the setting below; check its own docs for the commands.
2. Make it listen where Asha will look. By default that is
   `http://127.0.0.1:20128/v1`. If you run it on another host or port, edit
   `OMNIROUTE_BASE_URL` in `Asha.app/Contents/Resources/app/.env` to match.
3. Keep `JARVIS_BRAIN_TRANSPORT=omniroute` in that file (it is the default) so
   Asha routes the brain through the gateway.

Those are the exact names the app reads (`prototype/ui/server.py`,
`_resolve_brain_transport`) — `JARVIS_BRAIN_TRANSPORT` selects the transport and
`OMNIROUTE_BASE_URL` is the endpoint. Nothing is renamed and no code changes when
you point Asha at your own gateway.

**Where its state lives.** OmniRoute's database, secrets and logs live wherever
*your* OmniRoute install puts them (commonly its own data directory, e.g.
`~/.omniroute/`), not inside `Asha.app`. Asha does not manage, start, stop or
configure your OmniRoute — it only calls the URL you give it.

**How to check it.** Asha's own startup log prints exactly one line naming what
the brain resolved to (never a key); see
[Where the brain connects](#where-the-brain-connects-transport-settings) below.
On the OmniRoute side, version 3.8.50 answered `GET /api/health` with `200` and
served its dashboard at `http://127.0.0.1:20128/`, but use whatever health
endpoint your version documents.

**Licences.** OmniRoute is MIT. Because it is not shipped in the app, its
licence and its dependencies' licences are between you and that install; the
full text is in the OmniRoute package you install.

**If you do not want it**, do nothing — the app simply tries the transport you
configured. If `omniroute` is selected but nothing is listening, switch
`JARVIS_BRAIN_TRANSPORT` to `deepseek` or `opencode` and supply your own key (see
the table below). The package itself is never modified; you can also just edit
that one line back.
...
2. **`JARVIS_BRAIN_TRANSPORT`** — an explicit transport selector:
   `omniroute`, `deepseek`, or `opencode`.
3. **Auto** — `omniroute` for the packaged app; `opencode` when running from
   source.

| Transport | Setting | Base URL (default) | Key variable |
|---|---|---|---|
| Local gateway | `JARVIS_BRAIN_TRANSPORT=omniroute` | `http://127.0.0.1:20128/v1` (`OMNIROUTE_BASE_URL`) | `OMNIROUTE_API_KEY` (optional) |
| DeepSeek direct | `JARVIS_BRAIN_TRANSPORT=deepseek` | `https://api.deepseek.com` (`DEEPSEEK_BASE_URL`) | `DEEPSEEK_API_KEY` |
...
- **Packaged app:** the template sets `JARVIS_BRAIN_TRANSPORT=omniroute`, i.e.
  the local gateway at `http://127.0.0.1:20128/v1`, so no personal OpenCode
  subscription endpoint is used by default. This is the add-on above; if you
  have not installed it, change this line to `deepseek` or `opencode`.
...
For OmniRoute, make sure that model name resolves to a provider in the gateway
(e.g. a model alias/combo); that is gateway configuration, not app code.
...
```text
[LLM] brain transport: transport=omniroute base=http://127.0.0.1:20128/v1 host=127.0.0.1:20128 model=deepseek-v4.1-flash key_env=OMNIROUTE_API_KEY
```
...
Check the `[LLM] brain transport:` line in the log (above). If it says
`transport=omniroute` and you have not installed OmniRoute, either install and
run it, or set `JARVIS_BRAIN_TRANSPORT=deepseek` (or `opencode`) with your own
key in `Asha.app/Contents/Resources/app/.env`. See

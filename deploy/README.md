# Asha — self-host on your own server

Run the full Asha assistant (voice + coding) on your own VPS, like Hermes.
Two paths:
...
Data (sessions, memory, skills, prefs) persists in the `asha_data` volume.

## B. systemd on a bare VPS (Debian/Ubuntu)
```bash
git clone <your repo> asha && cd asha
sudo bash deploy/install.sh
# then edit /opt/asha/prototype/.env with your key(s):
sudo systemctl restart asha
```
Installs system deps (espeak-ng for TTS, libgomp1 for onnxruntime), a venv with
all packages, the voice models, and an `asha.service`. Open
`http://<server-ip>:8000`.
...
the app at it (Settings → provider) or use the built-in **plan** provider with
your gateway. (Remote-client mode in the packaged app is a follow-up.)
...
# or, systemd: sudo cp -R prototype /opt/asha/ && sudo systemctl restart asha
```

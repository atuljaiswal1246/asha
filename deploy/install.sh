#!/usr/bin/env bash
# Jarvis — self-host installer for a Linux VPS (Debian/Ubuntu).
#
...
# and a systemd unit. Then: edit /opt/jarvis/prototype/.env with your key(s)
# and `systemctl restart jarvis`. Open http://<server-ip>:8000.
set -euo pipefail

PREFIX="${JARVIS_PREFIX:-/opt/jarvis}"
...
[Unit]
Description=Jarvis assistant (voice + coding)
...
cat <<EOF
  ...
  Jarvis is running.
   ...
   1. put your key in:  $ENV          (then: sudo systemctl restart jarvis)
   2. open:             http://<server-ip>:$PORT_UI
   3. logs:             journalctl -u jarvis -f

...
Two ways to run Asha on Windows. Both start the same local Python backend
...
2. In the repo root, create the venv and install deps:
   ...
   ```
3. (Optional) Add your API key(s): open Asha → **Settings → API keys**, or drop
   them in `prototype\.env` (gitignored). Custom OpenAI-compatible endpoints go in
   the **Custom providers** section of Settings.

## 1. Quick run (no build) — `start_asha.bat`
Double-click `start_asha.bat`. It launches the backend and opens the UI in an
...
dist\Asha.exe
```
`Asha.exe` starts the backend itself and shows the UI in an embedded window
...
## Troubleshooting
- **"Could not find the Asha repo"** — set `JARVIS_HOME` to the repo folder

@echo off
REM Jarvis — no-build launcher for Windows.
...
echo [jarvis] starting backend: %PY% launch.py
start "Jarvis backend" /min cmd /c ""%PY%" "%UI%\launch.py" --no-browser"

echo [jarvis] waiting for the UI on http://127.0.0.1:8000 ...

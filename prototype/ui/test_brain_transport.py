...
* the packaged default is the DeepSeek-direct path (one key, no router);
* the optional OmniRoute gateway still resolves when explicitly selected;
...
class ResolveBrainTransportTests(unittest.TestCase):
    ...
    def test_packaged_default_is_deepseek_direct(self):
        # A packaged install with no explicit setting now behaves exactly like
        # the docs: one direct provider path, no local router. The key may be
        # absent; the resolver never crashes on that.
        ...
        self.assertEqual(info["key_env"], "")

    def test_explicit_omniroute_still_resolves(self):
        # OmniRoute is optional, but selecting it explicitly must keep working.
        info = server._resolve_brain_transport({
        ...
        self.assertEqual(info["source"], "JARVIS_BRAIN_TRANSPORT")
